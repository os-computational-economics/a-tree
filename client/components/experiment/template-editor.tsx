...
import { TEMPLATE_KINDS, isStaticBlock, isInformationBlock, isAiChatBlock, isSurveyBlock } from "@/lib/experiment/types";
...
function resolveBlockTemplate(config: ExperimentConfig, blockIndex: number, kind: TemplateKind): string {
  ...
  if (!block || isStaticBlock(block) || isInformationBlock(block) || isAiChatBlock(block) || isSurveyBlock(block)) return config[field];
...
export function TemplateEditor({ config, onChange }: TemplateEditorProps) {
  ...
  const hasBlockOverride = (block: ExperimentConfig["blocks"][number]) =>
    !isStaticBlock(block) && !isInformationBlock(block) && !isAiChatBlock(block) && !isSurveyBlock(block) && TEMPLATE_KINDS.some((k) => block[TEMPLATE_FIELD_MAP[k]]);
