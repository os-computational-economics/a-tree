...
function mergeParams(
  ...
  if (block && !isStaticBlock(block) && !isInformationBlock(block) && !isAiChatBlock(block) && !isSurveyBlock(block) && block.params) {
...
export function resolveFullRun(
  ...
  for (let bi = 0; bi < config.blocks.length; bi++) {
    ...
    if (isStaticBlock(block) || isInformationBlock(block) || isAiChatBlock(block) || isSurveyBlock(block)) continue;
